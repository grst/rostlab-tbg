...
{
	return menuWithArray(items,cols,rows,pos,pad,false, 0);
}

SlidingMenuGrid* SlidingMenuGrid::menuWithArray(CCArray *items, int cols, int rows, CCPoint pos , CCPoint pad, bool vertical, float threshold)
{
	SlidingMenuGrid *slidingMenu = new SlidingMenuGrid();
	if (slidingMenu && slidingMenu->initWithArray(items ,cols ,rows ,pos ,pad ,vertical, threshold))
	{
	...
	if( bMoving && !bFreezed )
	{
		bMoving = false;
		{
			...
			if( iPageCount > 1 && (fMoveDeadZone < abs(fMoveDelta)))
			{
				...
				if(bVerticalPaging)
				{
                    // Is vertical threshold reached?
                    if(abs(touchOrigin.y - touchStop.y) > fThreshold)
                    {
				...
				{
                    // Is horizontal threshold reached?
                    if(abs(touchOrigin.x - touchStop.x) > fThreshold)
                    {
                        if(!bForward && (iPageCount>iCurrentPage+1))
                        {
                            // Increment currently active page.
...
void SlidingMenuGrid::moveToCurrentPage(bool animated)
{
    ...
    if (animated)
    {
        CCMoveTo *move = CCMoveTo::create(fAnimSpeed*0.3f, GetPositionOfCurrentPage());
        CCEaseBounce* action =CCEaseBounce::create(move);
        runAction(action);
    }
    else
    {
